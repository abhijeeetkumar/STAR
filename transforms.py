...
class GroupScale(object):
    ...
    def __call__(self, img_group):
        ...
        elif img_group[0].shape[2] == 5:
            ret_img = []
            for img in img_group:
                new_img = img[...,0:3].astype('float64')
                new_img1 = img[..., 3:5].astype('float64')
                new_img = cv2.resize(new_img, self._size, cv2.INTER_LINEAR)
                new_img = np.dstack((new_img, resize_mv(new_img1, self._size, cv2.INTER_LINEAR))) 
...
class GroupOverSample(object):
    ...
    def __call__(self, img_group):
        ...
        image_w = img_group[0].shape[0]
        image_h = img_group[1].shape[1] 
    ...
    def __call__(self, img_group):
        ...
        if crop_img_group[0].shape[2] == 3:
            ...
            ret_img_group = [resize_mv(img, (self.input_size[0], self.input_size[1]), cv2.INTER_LINEAR)
                             for img in crop_img_group]
        elif crop_img_group[0].shape[2] == 5:
            ret_img_group = []
            for img in crop_img_group:
                new_img = img[...,0:3].astype('float64') 
                new_img1 = img[...,3:5].astype('float64')
                new_img = cv2.resize(new_img, (self.input_size[0], self.input_size[1]), cv2.INTER_LINEAR)
                new_img = np.dstack((new_img, resize_mv(new_img1, (self.input_size[0], self.input_size[1]), cv2.INTER_LINEAR)))
                ret_img_group.append(new_img)
            
            '''ret_img_group = [np.dstack((cv2.resize(img[...,0:3], (self.input_size[0], self.input_size[1]),
                                        cv2.INTER_LINEAR),
                                        resize_mv(img[...,3:5], (self.input_size[0], self.input_size[1]), cv2.INTER_LINEAR)))
                             for img in crop_img_group]'''
