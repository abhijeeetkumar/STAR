...
static struct PyModuleDef HERmodule = {
...
PyMODINIT_FUNC PyInit_HER(void)
{
    PyObject *m;

    m = PyModule_Create(&HERmodule);
    if (m == NULL)
        return NULL;
    ...
    HERError = PyErr_NewException("HER.error", NULL, NULL);
    Py_INCREF(HERError);
    PyModule_AddObject(m, "error", HERError);
    return m;
}
...
int main(int argc, char *argv[])
{
    ...
    PyImport_AppendInittab("her", PyInit_HER);
